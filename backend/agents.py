...
# above). Populated by the `remember` tool (see tools.py) as the agent works, so
# a later session in the same project starts already knowing things it learned
# before — conventions it discovered, gotchas, fixes that worked. Wrapped with
# language that tells the model these are its own past notes (useful context,
# not a directive) rather than an instruction from the user.
_LEARNED_MEMORY_FILE = ".coder/MEMORY.md"
_LEARNED_MEMORY_MAX_BYTES = 8_000


def _load_learned_memory(root: str) -> str:
    """Read the agent's self-written memory file, if present.

    Returns a ready-to-append system-prompt section, or ``""`` if the file
    doesn't exist yet (nothing has been remembered). Never raises.
    """
    try:
    ...
    if not body:
        return ""
    if len(body) > _LEARNED_MEMORY_MAX_BYTES:
        # Keep the MOST RECENT notes (newest entries are appended at the end),
        # not the oldest, since recent learnings are more likely still relevant.
        body = "…(older notes trimmed)\n" + body[-_LEARNED_MEMORY_MAX_BYTES:]
    return (
        f"\n\n===== YOUR OWN NOTES FROM EARLIER SESSIONS ({_LEARNED_MEMORY_FILE}) =====\n"
        "You wrote these yourself in past sessions on this project (via the remember tool): "
        "conventions you discovered, gotchas, fixes that worked, preferences the user mentioned "
        "in passing. Treat them as useful context, not absolute rules — if they conflict with "
        "AGENTS.md or the user's current message, those take priority.\n"
        f"{body}\n"
        "===== END YOUR OWN NOTES ====="
    )
